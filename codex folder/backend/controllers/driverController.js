...
async function registerDriver(req, res) {
    try {
        const { userId, licenseNumber, driverLicenseImage, spokenLanguages, hobbies, preferredMusic, gender } = req.body;
        if (!driverLicenseImage) return res.status(400).json({ error: "Driver license image is required" });
        ...
        const driver = await DriverProfile.create({
            userId, licenseNumber, driverLicenseImage, spokenLanguages, hobbies, preferredMusic, gender
        });

        const user = await User.findById(userId);
        const nextRole = user?.role === "passenger" ? "both" : (user?.role || "driver");
        await User.findByIdAndUpdate(userId, { role: nextRole });
